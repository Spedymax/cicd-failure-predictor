...
def test_post_commit_status_skips_synthetic_prefix() -> None:
    assert github_status.post_commit_status(
        "acme/widget", "deadbeef", PredictionDecision.BLOCK, 1,
    ) is False


def test_post_commit_status_skips_empty_or_malformed() -> None:
    assert github_status.post_commit_status(
        "", "sha", PredictionDecision.BLOCK, 1,
    ) is False
    assert github_status.post_commit_status(
        "noslash", "sha", PredictionDecision.BLOCK, 1,
    ) is False
...
def test_post_commit_status_success() -> None:
    ...
    with patch.object(
        ...
        assert github_status.post_commit_status(
            "Spedymax/cicd-predictor-demo", "abc" * 14,
            PredictionDecision.AUTO_APPROVE, 7,
        ) is True
